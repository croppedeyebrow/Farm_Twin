/**
 * 설비 제어 — 운전 기준 · 모드 · 수동 설비 · 이벤트 이력
 */

import { useEffect, useMemo, useState } from 'react'

import { ActuatorManualPanel } from '../components/ActuatorManualPanel'
...
export function EquipmentControlPage() {
  ...
  const activeCount = actuators.filter(
    (a) => a.mode !== 'off' && a.output_ratio > 0,
  ).length
    ...
    <main className="page page-ops" aria-label="설비 제어">
      ...
      <section className="ops-metrics ops-metrics-3" aria-label="제어 요약">
        ...
        <article className="ops-metric-card">
          ...
          <strong>
            {Math.max(activeCount, controlCount > 0 ? 1 : 0)}
            <span>대</span>
          </strong>
          <p>수동·자동 출력 &gt; 0</p>
        </article>
      ...
      <div className="ops-control-row">
        ...
        <section className="ops-panel" aria-label="운전 모드 · 설비">
          ...
          <p className="ops-hint">
            {driveMode === 'manual'
              ? '수동 모드: 아래에서 설비를 직접 ON/OFF·출력 조절합니다.'
              : '자동 모드: 운전 기준에 따라 설비가 갱신됩니다. 수동 조작도 가능합니다.'}
          </p>
        </section>
      </div>

      <section className="ops-panel" aria-label="수동 설비 제어">
        <header className="ops-panel-head">
          <h2>수동 설비 제어</h2>
          <span className="ops-sort-tag">{actuators.length}대</span>
        </header>
        <ActuatorManualPanel
          className="ops-actuator-manual"
          actuators={actuators}
          state={state}
